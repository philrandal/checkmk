...
if "%dateval%"=="" (
  ...
  if "%PROCESSOR_ARCHITECTURE%" == "AMD64" (
    for /f "eol=S skip=2 tokens=3" %%a in ('reg query "HKLM\SOFTWARE\Wow6432Node\McAfee\AvEngine" /v AVDatDate 2^>nul') do @set dateval=%%a
  )
  rem # on 32 bit systems
  if "%PROCESSOR_ARCHITECTURE%" == "x86" (
    for /f "eol=S skip=2 tokens=3" %%a in ('reg query "HKLM\SOFTWARE\McAfee\AvEngine" /v AVDatDate 2^>nul') do @set dateval=%%a
  )
)
